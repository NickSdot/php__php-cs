...
final readonly class ExpectedOutputUpdater
{
    ...
    private function updatedExpectLines(array $expectedLines, array $actualLines): ?array
    {
        ...
        while ($expectedIndex < count($expectedLines) && $actualIndex < count($actualLines)) {
            ...
            if ($this->canInsertActualBlankExpectLine($expectedLines, $actualLines, $expectedIndex, $actualIndex)) {
                $updated[] = $actualLine;
                $actualIndex++;
                continue;
            }
    ...
    private function updatedExpectfLines(array $expectedLines, array $actualLines): ?array
    {
        ...
        while ($expectedIndex < count($expectedLines) && $actualIndex < count($actualLines)) {
            ...
            if ($this->canInsertActualBlankExpectfLine($expectedLines, $actualLines, $expectedIndex, $actualIndex)) {
                $updated[] = $actualLine;
                $actualIndex++;
                continue;
            }
     ...
     * @param list<string> $actualLines
     */
    private function canInsertActualBlankExpectLine(array $expectedLines, array $actualLines, int $expectedIndex, int $actualIndex): bool
    {
        if ('' !== $actualLines[$actualIndex]) {
            return false;
        }

        $expectedLine = $expectedLines[$expectedIndex] ?? null;
        $actualLine = $actualLines[$actualIndex + 1] ?? null;
        ...
        return $expectedLine === $actualLine
            || $this->lineCanNormaliseToActual($expectedLine, $actualLine, exact: true);
    }
     ...
     * @param list<string> $actualLines
     */
    private function canInsertActualBlankExpectfLine(array $expectedLines, array $actualLines, int $expectedIndex, int $actualIndex): bool
    {
        if ('' !== $actualLines[$actualIndex]) {
            return false;
        }
        ...
        return $this->expectfLineMatches($expectedLine, $actualLine)
            || null !== $this->updatedExpectfLine($expectedLine, $actualLine);
    }
    ...
    {
        $candidates = $this->trash->withoutLeadingTrashCandidates($line);
    ...
    private function normalisedExpectfLine(string $candidate, array $actual): string
    {
        ...
        if ($this->isFullLineRegex($candidateBody)) {
    ...
    private function isFullLineRegex(string $line): bool
    {
        return str_starts_with($line, '%r') && str_ends_with($line, '%r');
    }
